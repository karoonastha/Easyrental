   ...
   <div className='flex justify-between flex-wrap gap-2 sm:grid grid-cols-[2fr_2fr_1fr_1fr] lg:grid-cols-[0.5fr_2fr_2fr_1fr_1fr] px-6 py-3 bg-secondary border-b-1 border-slate-900/15 rounded-t-xl'>
        ...
        <div>
          {properties.map((property, index)=>(
            <div key={index} className='flex justify-between items-center flex-wrap gap-2 sm:grid grid-cols-[2fr_2fr_1fr_1fr] lg:grid-cols-[0.5fr_2fr_2fr_1fr_1fr] px-6 py-3 bg-secondary/5 text-gray-50 medium-14 font-semibold border-b-1 border-slate-900/15'>
             <div className='hidden lg:block'>{index + 1}</div>
              <div className='flexStart gap-x-2 rounded-lg'>
                <div className='overflow-hidden rounded-lg'>
                <img src={property.images?.[0]} alt={property.title} className='w-16 rounded-lg' />

                </div>
                <div className='line-clamp-2'>{property.title}</div>
              </div>
              <div className='line-clamp-2'>{property.address}</div>
              <div>{currency}{property.price?.rent}</div>
              <div>
